...
export function ouvirSala(codigo) {
  ...
  const unsub = onSnapshot(salaRef, (docSnap) => {
    ...
    if (data.Tabuleiro && data.JogadorDaVez) {
      ...
      // 🔧 CORREÇÃO BUG 1: Só redesenha o tabuleiro se a jogada foi do OPONENTE
      // (Ou seja, se o novo "JogadorDaVez" é a MINHA cor)
      if (serializeBoard(board) !== data.Tabuleiro && newCurrent === myColor) {
...
// 🔧 CORREÇÃO BUG 2: A função agora aceita 'isMultiCapture'
async function enviarJogadaFirebase(mv, isMultiCapture = false) {
  ...
  // 🔧 CORREÇÃO BUG 2: Se for multi-captura, o JogadorDaVez NÃO MUDA.
  // Se não for, ele MUDA.
  ...
  try {
    await updateDoc(salaRef, {
      ...
      JogadorDaVez: proximoJogador, // Usa a nova lógica
...
export function initBoard(){
  ...
  if (isOnline) {
    ...
    if (getPlayerColor() === WHITE) {
      // Envia o estado inicial, não é multi-captura
...
export function onSquareClick(e){
  // 🔧 CORREÇÃO BUG 1: A checagem de 'current' agora é local e funciona
...
function endTurn(mv){
  ...
  // 🔧 CORREÇÃO BUG 2: Passa o status de multi-captura para o Firebase
  if (!isMultiCapture) {
    if (isOnline) {
      enviarJogadaFirebase(mv, false); // É uma jogada normal
    }
    switchPlayer();
  } else {
    // É multi-captura, NÃO troca o jogador (não chama switchPlayer)
    if (isOnline) {
      enviarJogadaFirebase(mv, true); // Avisa que é multi-captura
    }
  }
}
