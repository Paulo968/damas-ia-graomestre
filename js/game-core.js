...
export async function registrarJogador(nome, pin) {
  ...
  try {
    const newPlayerData = {
      ...
      pin: btoa(pin), // 🔧 CORREÇÃO (Bug 7): Codifica o PIN
      elo: 1000,
      vitorias: 0,
      derrotas: 0
    };
...
export async function logarJogador(nome, pin) {
  ...
  try {
    ...
    // Se 'atob' falhar (ex: PIN antigo salvo em texto puro), nega o login
    console.error("Erro ao decodificar PIN:", e);
    return { success: false, message: "Erro de conta. Registre-se novamente." };
  }
...
export async function criarSalaFirebase(codigo, jogador1_id) {
  ...
  // 🔧 CORREÇÃO (Bug 1): Usa 'currentRoom'
  // (Na verdade, aqui 'codigo' é o correto, pois estamos criando a sala)
...
export async function entrarSalaFirebase(codigo, jogador2_id) {
  ...
  // (Aqui 'codigo' também é o correto, pois estamos entrando)
  const salaRef = doc(db, `salas`, codigo); 
...
export function ouvirSala(codigo) {
  ...
  // (Aqui 'codigo' é o correto, pois estamos ouvindo a sala pela primeira vez)
  const salaRef = doc(db, `salas`, codigo);
...
function aiMove(movesToConsider=null){
  ...
  // (Removido ui.updateFaceState() para simplificar)
